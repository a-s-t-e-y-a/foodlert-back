...
const createFinanceCashRegister = {
  body: Joi.object({
    ...
    time: Joi.string().lowercase().valid('breakfast', 'lunch', 'dinner', 'night'),
    registerBy: Joi.string().custom(objectId).required(),
...
const createTransfer = {
  body: Joi.object({
    ...
    method: Joi.string().lowercase().valid('cash on hand', 'bank deposit').required(),
    status: Joi.string().lowercase().valid('approved', 'pending', 'cancelled')
  })
};
...
const changeFinanceStatus = {
  body: Joi.object({
    status: Joi.string().lowercase().valid('pending', 'approved', 'cancelled').required()
  })
};
