...
const createInventoryOrder = {
  body: Joi.object({
    orderedBy: Joi.string().custom(objectId).required(),
    recievedBy: Joi.string().custom(objectId).required(),
    paymentBy: Joi.string().trim().lowercase().valid('cash', 'eftpos', 'credit').required(),
    notes: Joi.string().trim(),
    orderDate: Joi.string().trim().required(),
    deliveryDate: Joi.string().trim(),
    status: Joi.string().trim().lowercase().valid('open', 'ordered', 'cancelled'),
...
const updateInventoryOrder = {
  body: Joi.object({
    ...
    paymentBy: Joi.string().trim().lowercase().valid('cash', 'eftpos', 'credit'),
    notes: Joi.string().trim(),
