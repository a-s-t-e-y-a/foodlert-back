...
const createInventorySupplierCart = async (req, res, next) => {
  const err = validateSchema(req, inventorySupplierCartValidation.createInventorySupplierCart);
  if (err) return next(new ApiError(404, `${err.details[0].message}`));

  const { item, quantity, supQuantity, supsupQuantity, price, total } = req.body;
  const id = req.params.id;

  const createdInventorySupplierCart = await inventorySupplierCartService.createInventorySupplierCart({
    id,
    item,
    quantity,
    supQuantity,
...
const getAllInventorySupplierCart = async (req, res, next) => {
  ...
  const getAllInventorySupplierCart = await inventorySupplierCartService.getAllInventorySupplierCart({ id });
  res
    .status(httpStatus.CREATED)
    .send({ totalInventorySupplierCart: getAllInventorySupplierCart.length, getAllInventorySupplierCart });
};

const updateInventorySupplierCart = async (req, res, next) => {
  const err = validateSchema(req, inventorySupplierCartValidation.updateInventorySupplierCart);
  if (err) return next(new ApiError(404, `${err.details[0].message}`));

  const id = req.params.id;
  const cartId = req.params.cartId;

  const updatedInventorySupplierCart = await inventorySupplierCartService.updateInventorySupplierCart({
    id,
    cartId,
    body: req.body
  });
...
const deleteInventorySupplierCart = async (req, res, next) => {
  ...
  await inventorySupplierCartService.deleteInventorySupplierCart({ id, cartId });
