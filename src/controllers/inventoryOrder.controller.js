...
const createInventoryOrder = async (req, res, next) => {
  const err = validateSchema(
    req,
    inventoryOrderValidation.createInventoryOrder
  );
  ...
  const createdInventoryOrder =
    await inventoryOrderService.createInventoryOrder({
...
const getAllInventoryOrder = async (req, res, next) => {
  const getAllInventoryOrder =
    await inventoryOrderService.getAllInventoryOrder();
  res
    .status(httpStatus.CREATED)
    .send({
      totalInventoryOrder: getAllInventoryOrder.length,
...
const getInventoryOrder = async (req, res, next) => {
  ...
  const getInventoryOrder = await inventoryOrderService.getInventoryOrder({
    id,
  });
...
const updateInventoryOrder = async (req, res, next) => {
  const err = validateSchema(
    req,
    inventoryOrderValidation.updateInventoryOrder
  );
  if (err) return next(new ApiError(404, `${err.details[0].message}`));
  ...
  const updatedInventoryOrder =
    await inventoryOrderService.updateInventoryOrder({ id, body: req.body });
