...
const createInventorySupplier = async (req, res, next) => {
  const err = validateSchema(
    req,
    inventorySupplierValidation.createInventorySupplier
  );
  ...
  const createdInventorySupplier =
    await inventorySupplierService.createInventorySupplier({
      name,
      email,
      portalUrl,
      mobileNo,
...
const getAllInventorySupplier = async (req, res, next) => {
  const getAllInventorySupplier =
    await inventorySupplierService.getAllInventorySupplier();
  res
    .status(httpStatus.CREATED)
    .send({
      totalInventorySupplier: getAllInventorySupplier.length,
...
const getInventorySupplier = async (req, res, next) => {
  ...
  const getInventorySupplier =
    await inventorySupplierService.getInventorySupplier({ id });
...
const updateInventorySupplier = async (req, res, next) => {
  const err = validateSchema(
    req,
    inventorySupplierValidation.updateInventorySupplier
  );
  if (err) return next(new ApiError(404, `${err.details[0].message}`));
  ...
  const updatedInventorySupplier =
    await inventorySupplierService.updateInventorySupplier({
      id,
      body: req.body,
    });
