...
const auth = async (req, res, next) => {
  ...
  if (!token) {
    return res.status(httpStatus.UNAUTHORIZED).json({ authenticated: false, message: 'No token given' });
  ...
  try {
    ...
    return res.status(httpStatus.UNAUTHORIZED).json({ authenticated: false, message: 'Invalid token, login first!' });
  }
